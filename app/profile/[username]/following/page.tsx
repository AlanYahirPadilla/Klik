...
export default async function FollowingPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  // Check which following the current user is following
  ...
  const followingIdsSet = new Set(currentUserFollows?.map((f) => f.following_id) || [])
      ...
      <div className="mx-auto max-w-2xl">
        ...
        {following && following.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
            ...
            <p className="text-lg font-semibold text-muted-foreground mb-2">No está siguiendo a nadie</p>
            <p className="text-sm text-muted-foreground">@{username} aún no sigue a nadie</p>
          </div>
        ) : (
          <div className="divide-y divide-border/40">
            {following
              ...
              .map((follow) => {
                ...
                const isFollowing = followingIdsSet.has(followProfile.id)
                const isOwnProfile = followProfile.id === currentUser.id
